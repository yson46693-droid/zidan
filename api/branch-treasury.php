...
if ($method === 'GET') {
    ...
    // ✅ حساب صافي رصيد الخزنة - السماح بالرصيد السالب
    // المعادلة: الرصيد = (الإيرادات + الإضافات) - (المصروفات + التكاليف + الخسائر + السحوبات)
    // قيمة إيجابية تعني ربح، قيمة سالبة تعني خسارة (يُسمح بالرصيد السالب)
    if ($isFirstBranch) {
        // الفرع الأول: المعادلة المحدثة
        // صافي رصيد الخزنة = (إجمالي الإيرادات + إجمالي الإيداعات + إجمالي تحصيلات الدين) - (إجمالي مصروفات الفرع + إجمالي تكاليف عمليات الصيانة + إجمالي العمليات الخاسرة + إجمالي السحوبات + إجمالي المرتجعات التالفة + إجمالي المرتجعات السليمة)
        $netBalance = ($totalRevenue + $totalDeposits + $totalDebtCollections) - ($totalExpenses + $totalRepairCosts + $totalLosses + $totalWithdrawals + $totalDamagedReturns + $totalNormalReturns);
        
        // ✅ تسجيل للتشخيص
        error_log("🔍 [Branch Treasury Debug] الفرع الأول - حساب الرصيد:");
        ...
        error_log("   - الخسائر: {$totalLosses}");
        error_log("   - السحوبات: {$totalWithdrawals}");
        error_log("   - المرتجعات التالفة: {$totalDamagedReturns}");
        error_log("   - المرتجعات السليمة: {$totalNormalReturns}");
        error_log("   - الرصيد الصافي: {$netBalance}");
    } else {
        ...
        // ✅ تسجيل للتشخيص
        error_log("🔍 [Branch Treasury Debug] الفرع الثاني - حساب الرصيد:");
        error_log("   - الإيرادات: {$totalRevenue}");
        error_log("   - الإيداعات: {$totalDeposits}");
        error_log("   - تحصيلات الدين: {$totalDebtCollections}");
        error_log("   - المصروفات: {$totalExpenses}");
        error_log("   - تكاليف الصيانة: {$totalRepairCosts}");
        error_log("   - الخسائر: {$totalLosses}");
        error_log("   - مسحوبات الرواتب: {$totalSalaryWithdrawals}");
        error_log("   - سحوبات الخزنة: {$totalTreasuryWithdrawals}");
        error_log("   - الرصيد الصافي: {$netBalance}");
    }
