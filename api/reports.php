...
if ($method === 'GET') {
    ...
    // 2. تكلفة المخزن (تكلفة الشراء الكلية)
    // ✅ ملاحظة: المخزن مشترك بين الفروع، لذلك لا نفلتره حسب الفرع
    // (المخزن هو تكلفة شراء عامة وليست مصروفات فرع محدد)
    // ⚠️ مهم: تكلفة المخزن لا تُضاف للمصروفات - تُخصم فقط عند البيع (في $totalSalesCost)
    ...
    // ❌ لا نضيف تكلفة المخزن للمصروفات
    // تكلفة المخزن تُخصم فقط عند البيع (في حساب $totalSalesCost)
    ...
    // حساب صافي ربح المبيعات من نقطة البيع
    $salesProfit = 0;
    $totalSalesRevenue = 0;
    $totalSalesCost = 0;
